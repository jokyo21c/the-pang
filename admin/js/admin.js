...
async function adminDownloadQuotePDF(orderId) {
    try {
        ...
        wrapper.innerHTML = buildQuotePdfHtml(order);
        document.body.appendChild(wrapper);
        const el = document.getElementById('quotePdfContent');
...
async function adminDownloadContractPDF(orderId) {
    try {
        ...
        const wrapper = document.createElement('div');
        wrapper.style.cssText = 'position:fixed; left:-9999px; top:0; z-index:99999; background:#fff;';
        wrapper.innerHTML = buildContractPage1Html(order) + buildContractPage2Html(order);
        document.body.appendChild(wrapper);
        const { jsPDF } = window.jspdf;
        const pdf = new jsPDF('p', 'mm', 'a4');
        ...
        const imgW = pdfW - 20;
        const c1 = await html2canvas(document.getElementById('pdfContractP1'), { scale: 2, useCORS: true, backgroundColor: '#fff' });
        pdf.addImage(c1.toDataURL('image/png'), 'PNG', 10, 10, imgW, Math.min((c1.height * imgW) / c1.width, pdfH - 20));
        pdf.addPage();
        const c2 = await html2canvas(document.getElementById('pdfContractP2'), { scale: 2, useCORS: true, backgroundColor: '#fff' });
