...
async function adminDownloadQuotePDF(orderId) {
    try {
        ...
        let basePriceNum = 0;
        if (basePrice.includes('|')) { const p = basePrice.split('|'); basePriceClean = (p[2] || p[0]) + '원'; basePriceNum = extractNumber(p[2] || p[0]); }
        else { basePriceClean = basePrice + '원'; basePriceNum = extractNumber(basePrice); }
        let addonsRows = '';
        let addonsTotalNum = 0;
        if (order.addons && order.addons.length > 0) {
            addonsRows = order.addons.map(a => {
                addonsTotalNum += extractNumber(a.price || '');
        ...
        // 금액 내역 계산
        const supplyAmount = basePriceNum + addonsTotalNum;
...
async function adminDownloadContractPDF(orderId) {
    try {
        ...
        // 금액 내역 계산
        const supplyAmount2 = basePriceNum + addonsTotalNum;
        const vatAmount2 = Math.floor(supplyAmount2 * 0.1);
        const grandTotal2 = supplyAmount2 + vatAmount2;

        const breakdownHtml2 = `
    <table style="width:100%; border-collapse:collapse; margin-top:16px; margin-bottom:8px;">
        ...
        <tr style="background:#f9f9f9;"><td style="padding:7px 12px; border:1px solid #ddd; font-weight:600; font-size:11px;">공급가액 (VAT 별도)</td><td style="padding:7px 12px; border:1px solid #ddd; text-align:right; font-weight:600; font-size:11px;">${supplyAmount2.toLocaleString('ko-KR')}원</td></tr>
        <tr><td style="padding:7px 12px; border:1px solid #ddd; font-weight:600; font-size:11px;">부가세 (VAT 10%)</td><td style="padding:7px 12px; border:1px solid #ddd; text-align:right; color:#e53c11; font-size:11px;">${vatAmount2.toLocaleString('ko-KR')}원</td></tr>
        <tr style="background:#7b2fff;"><td style="padding:9px 12px; border:1px solid #7b2fff; font-weight:700; color:#fff; font-size:12px;">합계 (VAT 포함)</td><td style="padding:9px 12px; border:1px solid #7b2fff; text-align:right; font-weight:700; color:#fff; font-size:14px;">${grandTotal2.toLocaleString('ko-KR')}원</td></tr>
    </table>`;
