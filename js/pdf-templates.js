...
function buildQuotePdfHtml(order) {
    ...
    const breakdown = `<table style="width:100%;border-collapse:collapse;margin-top:20px;margin-bottom:8px;">
        ...
        <tr style="background:#f9f9f9;"><td style="padding:10px 12px;border:1px solid #ddd;font-weight:700;color:#111;font-size:14px;">합계 (VAT 포함)</td><td style="padding:10px 12px;border:1px solid #ddd;text-align:right;font-weight:700;color:#111;font-size:16px;">${total.toLocaleString('ko-KR')}원</td></tr>
        ${order.total_amount ? `<tr style="background:#f5f0ff;"><td style="padding:10px 12px;border:1px solid #e0d4f5;font-weight:700;color:#7b2fff;font-size:14px;">확정 금액</td><td style="padding:10px 12px;border:1px solid #e0d4f5;text-align:right;font-weight:700;color:#e53c11;font-size:16px;">${order.total_amount}원</td></tr>` : ''}
    </table>`;
...
function buildContractPage1Html(order) {
    ...
    const totalAmt = order.total_amount ? (() => { const n = _pdfExtractNum(String(order.total_amount)); return n.toLocaleString('ko-KR'); })() : '0';
...
function buildContractPage2Html(order) {
    ...
    const breakdown2 = `<table style="width:100%;border-collapse:collapse;margin-top:16px;margin-bottom:8px;">
        ...
        ${order.total_amount ? `<tr style="background:#f5f0ff;"><td style="padding:9px 12px;border:1px solid #e0d4f5;font-weight:700;color:#7b2fff;font-size:14px;">계약 확정 금액</td><td style="padding:9px 12px;border:1px solid #e0d4f5;text-align:right;font-weight:700;color:#e53c11;font-size:14px;">${order.total_amount}원</td></tr>` : ''}
    </table>`;
